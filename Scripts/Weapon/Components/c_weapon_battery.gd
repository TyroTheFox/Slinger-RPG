...
func spend_energy(rate: float, delta: float) -> bool:	
	...
	if transaction_successful:
		...
		print("Spending: ", current_full_battery_capacity, " / ", current_battery_count)
...
func recharge_battery(_delta: float):
	...
	print("Recharging: ", current_full_battery_capacity, " / ", current_battery_count)

func update_ui():
	var remaining_full_battery_total = (battery_count - 1) * battery_capacity
